...
class Lab2bTab:
    def __init__(self,parent,ble):
        ...
        # Remember last FIND_BIAS results so STEP_RESPONSE can compute kV/kA.
        self.bias_L_mV=None
        self.bias_R_mV=None
        # Track which Lab2b action initiated this recording (step/ramp/bias)
        ...
        # Show more lines: step-response now reports τ and suggested #defines
        self.res_txt=make_ro(res,8,NEON_YELLOW);self.res_txt.pack(fill=tk.X)
        wr(self.res_txt,"  ARM → test → plots update")

    def do_step(self,s):
        self._last_test='step'
        self._start_rec();self.ble.send_cmd(cmd_step_response(self.step_mv.get(),self.step_dur.get(),s))
    def do_ramp(self,s):
        self._last_test='ramp'
        self._start_rec();self.ble.send_cmd(cmd_ramp_up(self.ramp_s.get(),self.ramp_e.get(),self.ramp_st.get(),self.ramp_d.get(),s))
    def do_bias(self):
        self._last_test='bias'
        self._start_rec();self.ble.send_cmd(cmd_find_bias())
    ...
    @staticmethod
    def _dt_ms(t_ms, t0_ms):
        """u16 time delta in ms (handles wrap-around)."""
        return (int(t_ms) - int(t0_ms) + 65536) % 65536

    @staticmethod
    def _find_on_off(d, cmd_key, cmd_on_mv=50):
        """Find first cmd ON edge and the corresponding OFF edge index.
        Returns (i_on, i_off) where i_off is first index after ON where cmd is OFF.
        If OFF not found, i_off = len(d)."""
        i_on=None
        for i,x in enumerate(d):
            c=abs(int(x.get(cmd_key,0)))
            if c>=cmd_on_mv and (i==0 or abs(int(d[i-1].get(cmd_key,0)))<cmd_on_mv):
                i_on=i;break
        if i_on is None:
            return None,None
        i_off=len(d)
        for j in range(i_on+1,len(d)):
            c=abs(int(d[j].get(cmd_key,0)))
            if c<cmd_on_mv and abs(int(d[j-1].get(cmd_key,0)))>=cmd_on_mv:
                i_off=j;break
        return i_on,i_off

    def _tau_for_side(self, d, cmd_key, vel_key, vt=5, cmd_on_mv=50):
        """Estimate step-response tau (ms) from FAST logs using 63.2% rule.
        Assumes cmd is approximately constant between ON and OFF."""
        i_on,i_off=self._find_on_off(d,cmd_key,cmd_on_mv)
        if i_on is None: return None
        seg=d[i_on:i_off]
        if len(seg)<8: return None
        # Baseline vel just before step
        pre=d[max(0,i_on-10):i_on]
        v0=sum(x[vel_key] for x in pre)/len(pre) if pre else 0.0
        # Steady-state vel: last 20% of ON segment
        j0=i_on + int(0.8*len(seg))
        tail=d[j0:i_off]
        vss=sum(x[vel_key] for x in tail)/len(tail) if tail else float(seg[-1][vel_key])
        # Guard: must actually move
        if abs(vss - v0) < vt*2:
            return None
        v63=v0 + 0.632*(vss - v0)
        # Find first crossing after ON
        t0_ms=d[i_on]['t_ms']
        if vss >= v0:
            pred=lambda v: v>=v63
        else:
            pred=lambda v: v<=v63
        i63=None
        for k in range(i_on, i_off):
            if pred(d[k][vel_key]):
                i63=k;break
        if i63 is None:
            return None
        tau_ms=self._dt_ms(d[i63]['t_ms'], t0_ms)
        # Also compute cmd plateau for kV estimation
        c_tail=[abs(int(x[cmd_key])) for x in tail if abs(int(x[cmd_key]))>0]
        cmd_ss=sum(c_tail)/len(c_tail) if c_tail else abs(int(seg[len(seg)//2][cmd_key]))
        return {
            'tau_ms': int(tau_ms),
            't0_ms': int(t0_ms),
            't63_ms': int(d[i63]['t_ms']),
            'v0': float(v0),
            'vss': float(vss),
            'v63': float(v63),
            'cmd_ss': float(cmd_ss),
    ...
    def _analyze(self):
        d=self._rec
        if len(d)<5:
            wr(self.res_txt,"  ✖ Not enough data")
            return

        # Common quick stats
        lines=[f"  ◆ {len(d)} samples"]

        # ── FIND BIAS (triggered by the button) ──
        if self._last_test=='bias':
            VT=5
            bL=bR=None
            for x in d:
                if bL is None and abs(x['velL_mmps'])>VT and x['vCmdL_mV']!=0:
                    bL=abs(int(x['vCmdL_mV']))
                if bR is None and abs(x['velR_mmps'])>VT and x['vCmdR_mV']!=0:
                    bR=abs(int(x['vCmdR_mV']))
            if bL is not None:
                self.bias_L_mV=int(bL)
                lines.append(f"  Bias L: ~{self.bias_L_mV} mV")
            if bR is not None:
                self.bias_R_mV=int(bR)
                lines.append(f"  Bias R: ~{self.bias_R_mV} mV")
            if self.bias_L_mV and self.bias_R_mV:
                lines.append(f"  Bias AVG: ~{(self.bias_L_mV+self.bias_R_mV)//2} mV")
            wr(self.res_txt,"\n".join(lines))
            return

        # ── STEP RESPONSE: compute tau + (optional) FF_LINEAR_ACC_X100 ──
        if self._last_test=='step':
            L=self._tau_for_side(d,'vCmdL_mV','velL_mmps')
            R=self._tau_for_side(d,'vCmdR_mV','velR_mmps')

            # Display SS velocities (from ON segment tail) even if tau fails
            if L:
                lines.append(f"  SS Vel L: {L['vss']:+.0f} mm/s   Cmd~{L['cmd_ss']:.0f} mV")
            if R:
                lines.append(f"  SS Vel R: {R['vss']:+.0f} mm/s   Cmd~{R['cmd_ss']:.0f} mV")

            taus=[x['tau_ms'] for x in [L,R] if x]
            if taus:
                tau_ms=int(round(sum(taus)/len(taus)))
                lines.append(f"  τ (63%): L={L['tau_ms'] if L else '-'}  R={R['tau_ms'] if R else '-'}  → AVG={tau_ms} ms")
                lines.append(f"  Suggest: #define FF_LINEAR_TAU_MS {tau_ms}")
            else:
                lines.append("  ✖ τ not found (need a clean step + enough samples)")
                wr(self.res_txt,"\n".join(lines))
                return

            # Optional: compute kV/kA if we have bias from FIND_BIAS
            if self.bias_L_mV is not None and self.bias_R_mV is not None:
                def kv_kA(side, bias, tau_ms):
                    if not side: return None,None
                    vss=abs(side['vss'])
                    if vss < 1: return None,None
                    kv=max(0.0, (side['cmd_ss'] - bias)/vss)  # mV/(mm/s)
                    kA=kv*(tau_ms/1000.0)                   # mV/(mm/s^2)
                    return kv,kA
                tau_ms=int(round(sum(taus)/len(taus)))
                kvL,kAL=kv_kA(L,self.bias_L_mV,tau_ms)
                kvR,kAR=kv_kA(R,self.bias_R_mV,tau_ms)
                kAs=[x for x in [kAL,kAR] if x is not None]
                if kAs:
                    kA_avg=sum(kAs)/len(kAs)
                    acc_x100=int(round(100*kA_avg))
                    lines.append(f"  Using stored Bias: L={self.bias_L_mV} R={self.bias_R_mV} mV")
                    if kvL is not None: lines.append(f"  kV_L≈{kvL:.2f} mV/(mm/s)")
                    if kvR is not None: lines.append(f"  kV_R≈{kvR:.2f} mV/(mm/s)")
                    lines.append(f"  kA≈{kA_avg:.3f} mV/(mm/s²) → FF_LINEAR_ACC_X100≈{acc_x100}")
                    lines.append(f"  Suggest: #define FF_LINEAR_ACC_X100 {acc_x100}")
                else:
                    lines.append("  (Run FIND_BIAS first to compute FF_LINEAR_ACC_X100 here)")
            else:
                lines.append("  (Run FIND_BIAS first to compute FF_LINEAR_ACC_X100 here)")
...
class Lab5Tab:
    def __init__(self, parent, ble):
        ...
        self.activity_log = make_ro(sf, 6, NEON_GREEN); self.activity_log.pack(fill=tk.X)
        self._log_activity("Ready. ใช้ STEP ทีละ cell หรือ SEARCH วิ่งต่อเนื่อง")
    ...
    def do_search(self):
        ...
        self._log_activity(f"▶ SEARCH {'CONT' if mode else 'S&G'} — started")
        self.ble.send_cmd(cmd_search_run(mode=mode))
        self.frame.after(100, self._burst_fetch_maze)
        self.frame.after(500, self._burst_fetch_maze)
    def do_step(self):
        self._log_activity("▸ STEP — executing 1 cell")
        self.ble.send_cmd(cmd_search_step())
        # ★ Faster refresh: 100ms (warmup done) → 800ms (drive done) → 1500ms (settle)
        self.frame.after(100, self._burst_fetch_maze)
        self.frame.after(800, self._burst_fetch_maze)
        self.frame.after(1500, self._burst_fetch_maze)
    def do_return(self):
        self._log_activity("◁ RETURN — heading to start")
        self.ble.send_cmd(cmd_search_return())
        self.frame.after(100, self._burst_fetch_maze)
    def do_reset(self):
        self._log_activity("⟲ RESET — maze cleared")
        self.ble.send_cmd(cmd_search_reset())
    ...
    def _log_activity(self, msg):
        ...
        wr(self.activity_log, f"  [{ts}] {msg}")

    def _log_state_transition(self, prev, q):
        """Log meaningful state transitions for debug."""
        DN = self._dir_names
        state = q['search_state']
        x, y, h = q['x'], q['y'], q['heading']
        STATES = ['IDLE','WARMUP','READING','DECIDING','TURNING','DRIVING',
                  'DRIVING▸▸','GOAL!','RETURNING','DONE','ERROR']
        sn = STATES[state] if state < len(STATES) else '?'
        hn = DN[h] if h < 4 else '?'
        ec = q.get('error_code', 0)

        if state == 1:  # WARMUP
            self._log_activity(f"▶ WARMUP — sensor settle ({x},{y})")
        elif state == 3:  # DECIDING
            self._log_activity(f"◈ READ WALLS → DECIDING at ({x},{y}) hdg={hn}")
        elif state == 4:  # TURNING
            self._log_activity(f"↻ TURNING → face {hn} at ({x},{y})")
        elif state == 5:  # DRIVING
            self._log_activity(f"▸ DRIVE 1 cell → {hn} from ({x},{y})")
        elif state == 6:  # DRIVING_MULTI
            self._log_activity(f"▸▸ DRIVING MULTI → {hn} from ({x},{y})")
        elif state == 7:  # GOAL
            self._log_activity(f"★ GOAL REACHED at ({x},{y})! steps={q.get('steps',0)}")
        elif state == 9:  # DONE
            self._log_activity(f"✔ DONE — back at start ({x},{y})")
        elif state == 10:  # ERROR
            self._log_activity(f"✖ ERROR code={ec} at ({x},{y})")
        elif state == 0 and prev != 0:  # → IDLE (step complete)
            self._log_activity(f"● Step done → ({x},{y}) hdg={hn} step#{q.get('steps',0)}")
    ...
    def do_apply_config(self):
        """Send goal/start config to firmware."""
        try:
            self.ble.send_cmd(cmd_search_set_config(
                start_x=self.start_x.get(), start_y=self.start_y.get(),
                start_heading=self.start_hdg.get(),
                goal_x_min=self.goal_x_min.get(), goal_y_min=self.goal_y_min.get(),
                goal_x_max=self.goal_x_max.get(), goal_y_max=self.goal_y_max.get(),
                req_id=159))
            # Update maze view goal highlight
            self.maze_view.goal = (self.goal_x_min.get(), self.goal_y_min.get(),
                                   self.goal_x_max.get(), self.goal_y_max.get())
            self._compute_local_flood()
            self.maze_view._draw()
            self.cfg_status.config(text="⏳ Sending config...", fg=NEON_YELLOW)
        except Exception as e:
            self.cfg_status.config(text=f"✖ {e}", fg=NEON_RED)
    ...
    def handle_rsp(self, data):
        ...
        if rsp['status'] != RSP_OK:
            if 150 <= rid <= 199 or 1560 <= rid <= 1575:
                self._log_activity(f"✖ RSP error: {RSP_NAMES.get(rsp['status'], '?')}")
            return
        ...
        # ── Search QUERY (req_id=155) ── ★ Updates pose + status ──
            ...
            try:
                ...
                # Sync goal from firmware (on first query or after config)
                if q.get('goal_x_min', 0) > 0 or q.get('goal_x_max', 0) > 0:
                    gx0, gx1 = q['goal_x_min'], q['goal_x_max']
                    gy0, gy1 = q['goal_y_min'], q['goal_y_max']
                    self.maze_view.goal = (gx0, gy0, gx1, gy1)
